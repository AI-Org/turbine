function cartplotU_mod
% cartplot is used to open and examine CART data
% Dependencies: loadcarthdrU, loadcartU,
...
function PanelResize(src,evt)
 ...
 % COMMENTING TO CHECK IF NESTED FUNCTION ERROR GOES AWAY 
   refresh(f)
    
end

%p = is_function_handle(figResize);
%print p
p = is_function_handle(@figResize);
print p
...
% handles    structure with handles and user data (see GUIDATA)
   ...
   % path=get(openmenu1,'userdata');
    ...
    if ~isequal(file,0)
        ...
        %hdr = loadcarthdrU([path char(file(1))]);
        hdr = loadcarthdr2([path char(file(1))]);
        for i=1:6
